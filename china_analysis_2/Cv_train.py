...
print("training CV:")
y_pred_knn= knn_model.fit(X,y).predict(X)
y_pred_etr= etr_model.fit(X,y).predict(X)
y_pred_dtr= dtr_model.fit(X,y).predict(X)
y_pred_br= bagging_model.fit(X,y).predict(X)
y_pred_ar= adaboost_model.fit(X,y).predict(X)
y_pred_xg= xgb_model.fit(X,y).predict(X)
y_pred_lr= lr_model.fit(X,y).predict(X)
y_pred_rf= rf_model.fit(X,y).predict(X)
y_pred_gbr= gbr_model.fit(X,y).predict(X)
rmse_knn= np.sqrt(mean_squared_error(y,y_pred_knn))
rmse_dtr= np.sqrt(mean_squared_error(y,y_pred_dtr))
rmse_xg= np.sqrt(mean_squared_error(y,y_pred_xg))
rmse_etr= np.sqrt(mean_squared_error(y,y_pred_etr))
rmse_br= np.sqrt(mean_squared_error(y,y_pred_br))
rmse_ar= np.sqrt(mean_squared_error(y,y_pred_ar))
