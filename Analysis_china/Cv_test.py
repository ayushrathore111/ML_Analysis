...
import numpy as np
data= pd.read_excel("PCC-29.xlsx")
X= data.iloc[:,:-1]
y= data.iloc[:,-1]
...
mlp_model = MLPRegressor(random_state=1, max_iter=50)
...
models = [knn_model,etr_model, dtr_model, adaboost_model, bagging_model, xgb_model,lr_model,rf_model,gbr_model,mlp_model]
print("testing CV: ")
...
y_pred_mlp= mlp_model.fit(X_train,y_train).predict(X_test)
...
dataf= {
    "models":["knn","etr","dtr","br","ar","xg","lr","rf","gbr","mlp"],
    "CV_RMSE":[rmse_knn , rmse_etr,rmse_dtr,rmse_br,rmse_ar
            ,rmse_xg,rmse_lr,rmse_rf,rmse_gbr,rmse_mlp]
}
